...
namespace FarmProject.Controllers
{
    ...
    public class CustomerController : ControllerBase
    {
        ...
        [HttpGet]
        public IActionResult GetCustomers(int CompanyId)
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(ConnectionString))
                {
                    SqlCommand getCustomersCmd = new SqlCommand("SELECT CustomerId,CustomerName,CustomerPhoneNumber,CustomerEmail,CustomerAddress,CustomerStatus FROM Customers where CompanyId=@CompanyId", connection);

                    connection.Open();
                    getCustomersCmd.Parameters.AddWithValue("@CompanyId", CompanyId);
        ...
        public IActionResult GetActiveCustomers(int CompanyId)
        {
            using (SqlConnection connection = new SqlConnection(ConnectionString))
            {
                using (SqlCommand command = new SqlCommand("GetActiveCustomer", connection))
                {
